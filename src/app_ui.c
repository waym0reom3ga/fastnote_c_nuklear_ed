...
/* Create a nuklear context with a baked (CPU-only) default font.  Safe
 * without a display; the GLFW device uploads the atlas texture later. */
struct nk_context *ui_nk_new(void) {
    struct nk_context *ctx = calloc(1, sizeof(struct nk_context));
    if (!ctx)
        return NULL;
    ...
    if (nk_font_atlas_init_default(atlas) == 0) {
        free(atlas);
        nk_free(ctx);
        free(ctx);
        return NULL;
    ...
    struct nk_font *font = nk_font_atlas_bake(atlas, &w, &h,
                                              NK_FONT_ATLAS_RGBA32);
    nk_font_atlas_end(atlas, nk_handle_id(0), &w, &h);
    if (font) {
        nk_style_set_font(ctx, &font->handle);
        ctx->userdata.ptr = atlas; /* keep alive for GL upload */
    } else {
        free(atlas);
    }
    return ctx;
...
void ui_nk_free(struct nk_context *ctx) {
    if (!ctx)
        return;
    struct nk_font_atlas *atlas = ctx->userdata.ptr;
    if (atlas) {
...
void ui_run_frame(UIState *ui, struct nk_context *ctx) {
    if (nk_begin(ctx, "FastNote", nk_rect(0, 0, (float)ui->width,
                                          (float)ui->height), 0)) {
        /* toolbar */
        nk_layout_row_dynamic(ctx, 30, ui->n_toolbar);
        ...
        nk_layout_row_dynamic(ctx, (float)(ui->height - 40 - 22 - 24), 2);
        {
            ...
            const char *before = ui->app.doc.text;
            size_t before_len = strlen(before);
            doc_set_text(&ui->app.doc, ui->editor_text);
            if (strlen(ui->app.doc.text) != before_len ||
                strcmp(ui->app.doc.text, before) != 0)
                ui_rebuild_preview(ui);
        }
        nk_label_wrap(ctx, ui->preview_text, NK_TEXT_LEFT);
    ...
    if (ui->browser_open) {
        ...
        if (nk_group_begin(ctx, "entries", 0)) {
            ...
            for (size_t i = 0; i < ui->browser.n_entries; i++) {
                ...
                if (nk_selectable_label(ctx, b->name, NK_TEXT_LEFT)) {
                    browser_activate(&ui->browser, b->name);
                    ui->n_entry_rects = 0;
                    ui_browser_after_activate(ui);
                    break;
                }
                if (ui->n_entry_rects < ui->browser.n_entries) {
                    struct nk_rect r = nk_widget_bounds(ctx);
                    ui->entry_rects =
